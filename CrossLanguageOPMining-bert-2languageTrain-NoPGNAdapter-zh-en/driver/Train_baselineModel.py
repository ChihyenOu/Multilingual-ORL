...
def train(data, dev_data, test_data, labeler, vocab, config, bert): # Remove , language_embedder
   # NEW VERSION
    ...
    for epoch in range(config.train_epochs): # iter -> epoch; config.train_iters -> config.train_epochs
        ...
        for onebatch in data_iter(data, config.train_batch_size, True):
                ...
                pgnbert_hidden = bert(input_ids=bert_indices_tensor, token_type_ids=bert_segments_tensor, 
                                        bert_pieces=bert_pieces_tensor) # Remove , lang_embedding=lang_embedding
    ...
    max_value = max(record_exact_dev_scores)
    index_value = [index for index in range(len(record_exact_dev_scores)) if record_exact_dev_scores[index] == max_value]
    logging.info("Best DEV F1-score = ", max_value)
    print("Best DEV F1-score = ", max_value)
    index_value = index_value[0] if len(index_value) == 1 else False
    print("index of the Best DEV F1-score = ", index_value)
    logging.info("index of the Best DEV F1-score = ", index_value)
    print("check if the value of the index is equal to best score = ", record_exact_dev_scores[index_value])

    logging.info("Result for other development score at index ", index_value)
    logging.info(record_exact_dev_scores[index_value])
    logging.info(record_exact_dev_recalls[index_value])
    logging.info(record_exact_dev_precisions[index_value])
    logging.info(record_exact_dev_agent_scores[index_value])
    logging.info(record_exact_dev_agent_recalls[index_value])
    logging.info(record_exact_dev_agent_precisions[index_value])
    logging.info(record_exact_dev_target_scores[index_value])
    logging.info(record_exact_dev_target_recalls[index_value])
    logging.info(record_exact_dev_target_precisions[index_value])
    logging.info(record_binary_dev_scores[index_value])
    logging.info(record_binary_dev_recalls[index_value])
    logging.info(record_binary_dev_precisions[index_value])
    logging.info(record_binary_dev_agent_scores[index_value])
    logging.info(record_binary_dev_agent_recalls[index_value])
    logging.info(record_binary_dev_agent_precisions[index_value])
    logging.info(record_binary_dev_target_scores[index_value])
    logging.info(record_binary_dev_target_recalls[index_value])
    logging.info(record_binary_dev_target_precisions[index_value])
    logging.info(record_prop_dev_scores[index_value])
    logging.info(record_prop_dev_recalls[index_value])
    logging.info(record_prop_dev_precisions[index_value])
    logging.info(record_prop_dev_agent_scores[index_value])
    logging.info(record_prop_dev_agent_recalls[index_value])
    logging.info(record_prop_dev_agent_precisions[index_value])
    logging.info(record_prop_dev_target_scores[index_value])
    logging.info(record_prop_dev_target_recalls[index_value])
    logging.info(record_prop_dev_target_precisions[index_value])
    
    logging.info("Result for all test scores at the last index. ")
    logging.info(record_exact_test_scores[-1])
    logging.info(record_exact_test_recalls[-1])
    logging.info(record_exact_test_precisions[-1])
    logging.info(record_exact_test_agent_scores[-1])
    logging.info(record_exact_test_agent_recalls[-1])
    logging.info(record_exact_test_agent_precisions[-1])
    logging.info(record_exact_test_target_scores[-1])
    logging.info(record_exact_test_target_recalls[-1])
    logging.info(record_exact_test_target_precisions[-1])
    logging.info(record_binary_test_scores[-1])
    logging.info(record_binary_test_recalls[-1])
    logging.info(record_binary_test_precisions[-1])
    logging.info(record_binary_test_agent_scores[-1])
    logging.info(record_binary_test_agent_recalls[-1])
    logging.info(record_binary_test_agent_precisions[-1])
    logging.info(record_binary_test_target_scores[-1])
    logging.info(record_binary_test_target_recalls[-1])
    logging.info(record_binary_test_target_precisions[-1])
    logging.info(record_prop_test_scores[-1])
    logging.info(record_prop_test_recalls[-1])
    logging.info(record_prop_test_precisions[-1])
    logging.info(record_prop_test_agent_scores[-1])
    logging.info(record_prop_test_agent_recalls[-1])
    logging.info(record_prop_test_agent_precisions[-1])
    logging.info(record_prop_test_target_scores[-1])
    logging.info(record_prop_test_target_recalls[-1])
    logging.info(record_prop_test_target_precisions[-1])
...
def evaluate(data, labeler, vocab, outputFile):
    ...
    for onebatch in data_iter(data, config.test_batch_size, False, False):
        ...
        count = 0
        # lang_embedding = language_embedder(lang_ids)
        bert_hidden = bert(input_ids=bert_indices_tensor, token_type_ids=bert_segments_tensor, bert_pieces=bert_pieces_tensor) # Remove , lang_embedding=lang_embedding
...
if __name__ == '__main__':
    ...
    # PGNBERT
    train(data, dev_data, test_data, labeler, vocab, config, bert) # Remove
